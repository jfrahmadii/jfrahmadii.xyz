...
This folder holds the import-ready **star schema** for the dashboard, plus the
scripts that built it, and is the data project behind the published piece at
[`projects/ontario-grid-decarbonization.html`](../../projects/ontario-grid-decarbonization.html).
Connect Power BI Desktop to these CSVs and start modeling.

> The website's inline chart data is regenerated from the `Fact_Generation_ON_*.csv`
> files here by `tools/build-site-data.mjs` at the repo root:
